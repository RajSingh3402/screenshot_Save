...
// Import MySQL database connection & CRUD helpers
...
nextApp.prepare().then(async () => {
  // Initialize MySQL tables and import data from db.json if database is empty
  await db.initDb();
  ...
  app.post('/api/websites', async (req, res) => {
    try {
      ...
      await db.createWebsite(newSite);
  ...
  app.put('/api/websites/:id', async (req, res) => {
    try {
      ...
      const existingSite = await db.getWebsiteById(siteId);
      if (!existingSite) {
      ...
      const updatedSite = await db.updateWebsite(siteId, fieldsToUpdate);
  ...
  app.post('/api/websites/bulk', async (req, res) => {
    try {
      ...
      await db.bulkInsertWebsites(newSites);
  ...
  async function runCaptureSession(triggerName = 'Manual Trigger') {
    ...
    const captureWorker = async (site, index) => {
      ...
      try {
        await db.updateWebsite(site.id, {
...
`;
  ...
  cron.schedule('* * * * *', async () => {
    try {
      ...
      console.error('Error in cron background scheduler:', err);
    }
  });
